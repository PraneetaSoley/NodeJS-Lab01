			...
			var query = dbConn.query("select * from users where email_id=? and password=?", [email, pwd], function(err, rows) {
				...
				} else 
				{
					if (rows.length >= 1)
					{
						...
						//getExperience(localSession.user_id);
						//getEducation(localSession.user_id);
						//console.log(localSession);
						//sessions.addSessionToMap(localSession.user_id, localSession);
						//res.send({"login":"Success"});
					}else
					{
...
exports.updateExperience = function(req, res) {
	...
	console.log(localSession.user_id);
	if(idExperience === "0")
	{
...
//function signInVal(req, res){
//	
//	if(req==null)
//		{
//		alert("fields should not be empty");
//		}
//	else if(req.param("email")!=null && req.param("password")==null)
//		{
